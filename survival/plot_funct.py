...
def prettyplot(n_cv, i, survivalloc, analysis_directory, disease_type):
	...
	labels= ax.get_xticklabels(minor=False)
	...
	labelsv= axv.get_xticklabels(minor=False)
	axv.set_xticklabels(labelsv, rotation='vertical')
	#ax.arrow(0, 0, paramt[0][1][0][0][0], paramt[0][1][0][0][1], head_width=.3, head_length=.3, fc='k', ec='k')
	# ax.set_title('Volume and percent change')
	# ax.legend()
	axv.set_ylim(ylim)
	...
	for z1 in range(EZ.shape[0]):
		for z2 in range(z1 +1):
			if z2<z1:
				...
				labels_v= ax_v.get_xticklabels(minor=False)
				ax_v.set_xticklabels(labels_v, rotation='vertical')
				#ax.arrow(0, 0, paramt[0][1][0][0][0], paramt[0][1][0][0][1], head_width=.3, head_length=.3, fc='k', ec='k')
				# ax.set_title('Volume and percent change')
				fig_v.tight_layout()
