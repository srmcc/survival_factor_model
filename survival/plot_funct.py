...
def prettyplot(n_cv, i, survivalloc, analysis_directory, disease_type):
 ...
 #            bbox_inches='tight')
	...
	figv.colorbar(im, ax=axv)
	axv.set_xlabel(r'$E[z_1| x]$', fontsize=20)
	axv.set_ylabel(r'$E[z_2| x]$', fontsize=20)
	#ax.arrow(0, 0, paramt[0][1][0][0][0], paramt[0][1][0][0][1], head_width=.3, head_length=.3, fc='k', ec='k')
	# ax.set_title('Volume and percent change')
	# ax.legend()
	axv.set_ylim(ylim)
	axv.set_xlim(xlim)
	figv.tight_layout()
	plt.savefig(analysis_directory+'cv_'+str(n_cv)+ '_results/cv_run'+str(i)+'/model_0_0/val/'+ disease_type+ '_EZ_given_x_grey_eventtime.pdf',
	            bbox_inches='tight')
	plt.savefig(analysis_directory+'cv_'+str(n_cv)+ '_results/cv_run'+str(i)+'/model_0_0/val/'+ disease_type+ '_EZ_given_x_grey_eventtime.eps', format='eps', dpi = 1200,
	            bbox_inches='tight')
		...
		for z2 in range(z1 +1):
			if z2< z1:
				...
				for k, m in enumerate(markers):
					...
					im=ax.scatter(x[j], y[j], c=t[j], cmap= cm, marker=m, s=100, vmin= vminn, vmax = vmaxx, edgecolors= 'black')
				fig.colorbar(im, ax=ax)
				ax.set_xlabel(r'$E[z_%d| x]$' % (z2 +1), fontsize=20)
				ax.set_ylabel(r'$E[z_%d| x]$' %(z1 +1), fontsize=20)
				#ax.arrow(0, 0, paramt[0][1][0][0][0], paramt[0][1][0][0][1], head_width=.3, head_length=.3, fc='k', ec='k')
				# ax.set_title('Volume and percent change')
				# ax.legend()
				fig.tight_layout()
	...
	for z1 in range(EZ.shape[0]):
		for z2 in range(z1 +1):
			if z2<z1:
				...
				figv.colorbar(im_v, ax=axv)
				axv.set_xlabel(r'$E[z_%d| x]$' %(z2 +1), fontsize=20)
				axv.set_ylabel(r'$E[z_%d| x]$' %(z1 +1), fontsize=20)
				#ax.arrow(0, 0, paramt[0][1][0][0][0], paramt[0][1][0][0][1], head_width=.3, head_length=.3, fc='k', ec='k')
				# ax.set_title('Volume and percent change')
				# ax.legend()
				axv.set_ylim(ylim)
			...
			if z2<z1:
				...
				for l in range(colorcat_v.shape[0]):
					...
					for k, m in enumerate(markers):
						...
						# 	print(k, l, colorlist[l], x[j].shape)
				#fig.colorbar(im, ax=ax)
				ax_v.legend(leg_v, names_v, bbox_to_anchor=(1.05, 1), loc=2, borderaxespad=0.)
				ax_v.set_xlabel(r'$E[z_%d| x]$'%(z2 +1), fontsize=20)
				ax_v.set_ylabel(r'$E[z_%d| x]$'%(z1 +1), fontsize=20)
				#ax.arrow(0, 0, paramt[0][1][0][0][0], paramt[0][1][0][0][1], head_width=.3, head_length=.3, fc='k', ec='k')
				# ax.set_title('Volume and percent change')
				fig_v.tight_layout()
	...
	for z1 in range(trueZ.shape[0]):
		for z2 in range(z1 +1):
			if z2<z1:
				...
				#fig.colorbar(im, ax=ax)
				ax_v.legend(leg_v, names_v, bbox_to_anchor=(1.05, 1), loc=2, borderaxespad=0.)
				ax_v.set_xlabel(r'$ z_%d$'%(z2 +1), fontsize=20)
				ax_v.set_ylabel(r'$ z_%d$'%(z1 +1), fontsize=20)
				#ax.arrow(0, 0, paramt[0][1][0][0][0], paramt[0][1][0][0][1], head_width=.3, head_length=.3, fc='k', ec='k')
				# ax.set_title('Volume and percent change')
				fig_v.tight_layout()
