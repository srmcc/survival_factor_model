...
def make_table2(root_directory, gold_standard):
	...
	for disease_type in ["SIM1", "SIM2", "SIM3", "SIM4"]:
		row_names = row_names + 2* [disease_type]
	if gold_standard== True:
		Table2 = pd.DataFrame(np.zeros((20, 5)), columns =["Disease", "Model no.", "Test Set c-index", "CV c-index mean", "CV c-index standard deviation"], index = row_names) 
	else:
	...
	for disease_type in ["LGG", "GBM", "LUAD", "LUSC", "SIM1", "SIM2", "SIM3", "SIM4"]:
		...
		if disease_type in ["LGG", "GBM", "LUAD", "LUSC"] and gold_standard == True:
			Table2.loc[disease_type, "Disease"] = [mod, 4+ mod_cox, 7]
			Table2.loc[disease_type, "CV c-index mean"] = [means[mod], means[4+ mod_cox], means[7]]
			Table2.loc[disease_type, "CV c-index standard deviation"] = [stdvs[mod], stdvs[4+ mod_cox], stdvs[7]]
			with open(root_directory + 'project_' +disease_type +'/analysis/final_fit_results/model_selection_output_final.txt', 'r') as f:
				with open(root_directory + 'project_' +disease_type +'/analysis/final_fit_results_cox/model_selection_output_coxfinal.txt', 'r') as g:
					with open(root_directory + 'project_' +disease_type +'/analysis/final_fit_results_cox_gs/model_selection_output_coxfinal.txt', 'r') as h:
			...
			Table2.loc[disease_type, "Disease"] = [mod, 4+ mod_cox]
			Table2.loc[disease_type, "CV c-index mean"] = [means[mod], means[4+ mod_cox]]
			Table2.loc[disease_type, "CV c-index standard deviation"] = [stdvs[mod], stdvs[4+ mod_cox]]
			with open(root_directory + 'project_' +disease_type +'/analysis/final_fit_results/model_selection_output_final.txt', 'r') as f:
				with open(root_directory + 'project_' +disease_type +'/analysis/final_fit_results_cox/model_selection_output_coxfinal.txt', 'r') as g:
						Table2.loc[disease_type, "Test Set c-index"] = [np.round(eval(f.readline())[-1][0], 2), np.round(eval(g.readline())[-1][0], 2)]
